...
double * average_1(input_data *input, CLIENT *client) 
  ...
  sum_avg = sum_avg / input->input_data.input_data_len; //average
  dp = input->input_data.input_data_val; // reset pointer to first value of the array
  
  for ( i = 1; i <= input->input_data.input_data_len; i++) {
    hold = *dp - sum_avg; // find the distance from each data point to the mean
    // distance is always positive, so check for that
    hold = pow(hold, 2); // square the distance
    std_dev += hold; // sum the distances
    dp++;
  }
 
  std_dev = std_dev / input->input_data.input_data_len; // find average distance from mean
  //printf("average of distances = %lf", hold);
  sum_avg = sqrt(std_dev); // take the square root of the average
